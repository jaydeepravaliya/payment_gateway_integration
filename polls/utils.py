...
def get_redirect_url(request):
    url = f"{settings.DOMAIN}{reverse('dcavenue-callback')}"
...
def checksum(redirect_url, amount, order_id):
    ...
    data = "%s|%s|%s|%s|%s" % (
        settings.CCAVENUE["MERCHANT_ID"], order_id, amount,
        redirect_url, settings.CCAVENUE["WORKING_KEY"]
    )
...
def enc_request(request, order_id):
    ...
    cca_request = "&".join(
        [
            "Merchant_Id=%s" % settings.CCAVENUE["MERCHANT_ID"],
            "Amount=%s" % amount,
            "Order_Id=%s" % order_id,
    ...
    return commands.getoutput(
        '%s -jar %s %s "%s" enc' % (
            settings.CCAVENUE.get("JAVA", "java"), settings.CCAVENUE["JAR"],
            settings.CCAVENUE["WORKING_KEY"], cca_request
        )
    )
...
def verify_checksum(data):
    ...
    inp = "%s|%s|%s|%s|%s" % (
        settings.CCAVENUE["MERCHANT_ID"], data["Order_Id"], data["Amount"],
        data["AuthDesc"], settings.CCAVENUE["WORKING_KEY"]
    )
...
def dec_response(request, response):
    response = commands.getoutput(
        '%s -jar %s %s "%s" dec' % (
            settings.CCAVENUE.get("JAVA", "java"), settings.CCAVENUE["JAR"],
            settings.CCAVENUE["WORKING_KEY"], response
        )
    )
